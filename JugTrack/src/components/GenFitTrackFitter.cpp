  ...
  StatusCode GenFitTrackFitter::execute()
  {
    ...
    for (int itrack = 0; itrack < std::min(n_tracks, n_proto_tracks); itrack++) {
      ...
      for (int ihit : proto_track) {
        ...
        auto local_position =
            alignment.worldToLocal({ahit.position().x / 10.0, ahit.position().y / 10.0, ahit.position().z / 10.0});
        auto surf = m_surfaceMap[vol_id];
        auto local_position2 =
            surf->globalToLocal({ahit.position().x / 10.0, ahit.position().y / 10.0, ahit.position().z / 10.0});
        ...
        if (msgLevel(MSG::DEBUG)) {
          debug() << "------------------------------------ " << endmsg;
          debug() << " hit position     : " << ahit.position().x/10 << " " << ahit.position().y/10 << " " << ahit.position().z/10 << endmsg;
