...
Jug::Reco::TestIRTAlgorithm::TestIRTAlgorithm(const std::string& name, ISvcLocator* svcLoc) 
...
} // Jug::Reco::TestIRTAlgorithm::TestIRTAlgorithm()

// -------------------------------------------------------------------------------------

StatusCode Jug::Reco::TestIRTAlgorithm::initialize( void ) 
{
...
} // Jug::Reco::TestIRTAlgorithm::initialize()

// -------------------------------------------------------------------------------------

StatusCode Jug::Reco::TestIRTAlgorithm::execute( void )
{
  // Input collection(s);
  ...
  for(const auto &mctrack: mctracks) {
	...
	if (trajectory && trajectory->points().size()) {
	  ...
	  for(const auto &point: trajectory->points()) 
	    ...
	    if (!best || TestIRTAlgorithmServices::GetDistance(surface, &point) < 
		TestIRTAlgorithmServices::GetDistance(surface, best))
	      best = &point;

	  // FIXME: well, may want to check how far away the best point actually is;
	  {
	    TVector3 from, to;
	    TestIRTAlgorithmServices::GetCrossing(s1->GetSurface(), best, &from);
	    TestIRTAlgorithmServices::GetCrossing(s2->GetSurface(), best, &to);
	    
	    auto p0 = TestIRTAlgorithmServices::GetMomentum(best);
  ...
  return StatusCode::SUCCESS;
} // Jug::Reco::TestIRTAlgorithm::execute()

// -------------------------------------------------------------------------------------

StatusCode Jug::Reco::TestIRTAlgorithm::finalize( void ) 
{
  info() << "TestIRTAlgorithm: Finalizing..." << endmsg;

  return Algorithm::finalize(); // must be executed last
} // Jug::Reco::TestIRTAlgorithm::finalize()
