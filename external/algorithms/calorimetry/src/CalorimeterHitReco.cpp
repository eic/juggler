...
void CalorimeterHitReco::init() {
  ...
  try {
    ...
    if (!m_sectorField.value().empty()) {
      ...
      info() << "Find sector field " << m_sectorField.value() << ", index = " << sector_idx
             << endmsg;
    }
...
#pragma GCC diagnostic error "-Wsign-conversion"
    ...
    // convert ADC -> energy
    const float energy = (((signed)rh.getAmplitude() - (signed)m_pedMeanADC)) /
                         static_cast<float>(m_capADC.value()) * dyRangeADC / m_sampFrac;
    const float time = rh.getTimeStamp() / stepTDC;

#pragma GCC diagnostic pop

    const auto cellID = rh.getCellID();
    const int lid     = id_dec != nullptr && !m_layerField.value().empty()
                            ? static_cast<int>(id_dec->get(cellID, layer_idx))
                            : -1;
    const int sid     = id_dec != nullptr && !m_sectorField.value().empty()
                            ? static_cast<int>(id_dec->get(cellID, sector_idx))
                            : -1;
    // global positions
    const auto gpos = converter->position(cellID);
    // local positions
    auto local_det = local;
    if (m_localDetElement.value().empty()) {
      auto volman = m_geo.detector()->volumeManager();
      local_det   = volman.lookupDetElement(cellID & local_mask);
    }
    const auto pos = local_det.nominal().worldToLocal(dd4hep::Position(gpos.x(), gpos.y(), gpos.z()));
    // cell dimension
    std::vector<double> cdim;
    // get segmentation dimensions
    if (converter->findReadout(local_det).segmentation().type() != "NoSegmentation") {
      cdim = converter->cellDimensions(cellID);
      // get volume dimensions (multiply by two to get fullsize)
    } else {
      // Using bounding box instead of actual solid so the dimensions are always in dim_x, dim_y,
      // dim_z
